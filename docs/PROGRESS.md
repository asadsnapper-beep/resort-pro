...
**T-42 — Role-Based Access Control (RBAC) সম্পূর্ণ ✅**

`UserRole` enum প্রসারিত: `OWNER | MANAGER | PARTNER | RECEPTIONIST | MARKETER | DEVELOPER | STAFF | GUEST`। Schema: `prisma db push` দিয়ে নতুন enum values লাইভ। **Sidebar** (`sidebar.tsx`): `NAV_ITEMS`-এ প্রতিটা item-এ `roles?: Role[]` property — `getVisibleItems(role)` filter করে শুধু allowed items দেখায়; groups collapsible (ChevronDown toggle); user footer-এ colored role badge (gold=OWNER, resort=MANAGER, blue=PARTNER, emerald=RECEPTIONIST, purple=MARKETER, gray=DEVELOPER, orange=STAFF)। **Permission matrix** (sidebar-level): OWNER+MANAGER সব দেখে; PARTNER শুধু Dashboard+Analytics; RECEPTIONIST — Rooms, Bookings, Calendar, Group Bookings, Packages, Guests, Loyalty, Support, Housekeeping, F&B Orders; MARKETER — Analytics, Guests, CRM, Website; DEVELOPER — Website, Settings; STAFF — Housekeeping, Maintenance, Restaurant, Orders, Inventory; Billing শুধু OWNER। **API routes** আপডেট: `bookings/packages/groupBookings/loyalty` → OWNER+MANAGER+RECEPTIONIST; `ratePlans/reports` → OWNER+MANAGER; `maintenance/inventory` → OWNER+MANAGER+STAFF; `website` → OWNER+MANAGER+MARKETER+DEVELOPER; `crm` → OWNER+MANAGER+MARKETER; feature flags read → সব roles; `requireRole('MANAGER','ADMIN')` pattern সব routes থেকে সরানো হয়েছে। **Staff Invite Modal**: 3×2 role grid (emoji + label + description hint) — Staff/Receptionist/Manager/Marketer/Developer/Partner। **Demo Seed**: 4 নতুন test user — `receptionist@palmparadise.com`, `partner@palmparadise.com`, `marketer@palmparadise.com`, `developer@palmparadise.com` (সব Password: `Password123!`)। 0 new TS errors (web + api)।
