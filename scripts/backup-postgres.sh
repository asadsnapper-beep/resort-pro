...
#
# One half of a backup. backup-db.sh runs this and backup-uploads.sh, and the
# two are kept apart deliberately: a database that cannot be reached is no
# reason to stop archiving guest documents, and the reverse holds too.
#
# Runs in the API image, which carries postgresql16-client, so nothing has to
