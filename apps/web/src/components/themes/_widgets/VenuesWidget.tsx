...
import { Users, Send, CheckCircle, X, Landmark } from 'lucide-react';
...
export function VenuesWidget({ slug, primaryColor, accentColor, currency }: WidgetProps) {
  const [venues, setVenues]   = useState<Venue[]>([]);
  const [loading, setLoading] = useState(true);
  const [enquiryVenue, setEnquiryVenue] = useState<Venue | null>(null);
  const [form, setForm]       = useState({ name: '', email: '', phone: '', preferredDate: '', guestCount: '', message: '' });
  const [submitting, setSubmitting] = useState(false);
  const [success, setSuccess] = useState(false);
  const [error, setError]     = useState('');

  useEffect(() => {
    fetch(`${API_BASE}/site/${slug}/venues`)
      .then(r => r.json())
      .then(j => { setVenues(j.data || []); setLoading(false); })
      .catch(() => setLoading(false));
  }, [slug]);

  const openEnquiry = (v: Venue) => {
    setEnquiryVenue(v);
    setForm({ name: '', email: '', phone: '', preferredDate: '', guestCount: '', message: '' });
  ...
  const submitEnquiry = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/site/${slug}/venue-enquiry`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          venueId: enquiryVenue!.id, name: form.name, email: form.email || undefined, phone: form.phone,
  ...
  return (
    <section id="venues" className="py-24 bg-stone-50">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-12">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] mb-3" style={{ color: accentColor }}>
            Venues & Events
          </p>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900">Host Your Event With Us</h2>
          <p className="mt-4 text-gray-500 max-w-xl mx-auto">
            Conference halls, banquet space, and outdoor venues available to rent
          </p>
        </div>
            ...
            <div key={v.id}
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <div className="flex items-center gap-1.5 text-sm text-gray-500 mt-1">
                  <Users className="h-3.5 w-3.5" /> Up to {v.capacity} guests
                </div>
                {v.description && <p className="text-sm text-gray-500 mt-2 line-clamp-2 leading-relaxed flex-1">{v.description}</p>}
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                  <span className="text-base font-bold" style={{ color: primaryColor }}>
                    {v.fullDayRate ? `${fmt(v.fullDayRate, currency)}/day` : v.hourlyRate ? `${fmt(v.hourlyRate, currency)}/hr` : 'Enquire'}
                  </span>
                  <button onClick={() => openEnquiry(v)}
                    className="px-4 py-2 rounded-full text-xs font-semibold text-white hover:opacity-90 hover:scale-105 transition-all"
      ...
      {enquiryVenue && (
        <>
          <div className="fixed inset-0 z-40 bg-black/30 backdrop-blur-sm" onClick={() => setEnquiryVenue(null)} />
            ...
            <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4" style={{ backgroundColor: primaryColor }}>
                <div className="flex items-center gap-2 text-white">
                  ...
                  <h3 className="font-bold">{enquiryVenue.name}</h3>
                </div>
                <button onClick={() => setEnquiryVenue(null)} className="text-white/80 hover:text-white">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="p-6">
                {success ? (
                  <div className="text-center py-6">
                    ...
                    <button onClick={() => setEnquiryVenue(null)} className="mt-6 px-6 py-2.5 rounded-full text-white font-semibold text-sm" style={{ backgroundColor: primaryColor }}>
                      Close
                    </button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1.5">Guest Count</label>
