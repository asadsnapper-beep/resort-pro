...
const TIER_CONFIG = {
  ...
  SILVER:   { label: 'Silver',   bg: 'var(--rp-surface-3)', border: 'var(--rp-border-md)',      text: 'var(--rp-text-muted)', icon: '🥈', bar: '#aac0d0' },
  GOLD:     { label: 'Gold',     bg: 'var(--rp-amber-bg)', border: 'rgba(184,144,64,0.2)',  text: '#b89040', icon: '🥇', bar: '#d4a853' },
  PLATINUM: { label: 'Platinum', bg: '#183153', border: 'rgba(24,49,83,0.5)',    text: '#f8fafc', icon: '💎', bar: '#183153' },
};

const TX_TYPE_CONFIG = {
  EARN:   { label: 'Earned',   color: '#183153', sign: '+' },
  ...
  const field = (key: keyof typeof form, label: string, type = 'text', hint?: string) => (
    <div>
      ...
      <input type={type} value={form[key] as string}
        ...
        className={inputCls} />
      {hint && <p className="mt-1 text-[11px] text-[#64748b] dark:text-[#a9c1d0]">{hint}</p>}
    </div>
  );
    ...
    >
      <div className="space-y-5">
          ...
          <div className="flex items-center justify-between rounded-[10px] border px-4 py-3"
            ...
            <div>
              <p className="text-[13px] font-medium text-[#183153] dark:text-[#f8fafc]">Enable Loyalty Program</p>
              <p className="text-[11.5px] mt-0.5 text-[#64748b] dark:text-[#a9c1d0]">Points auto-awarded on guest checkout</p>
            </div>
          ...
          <div>
            <p className="text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[#64748b] mb-3">Tier Thresholds (lifetime points)</p>
            <div className="grid grid-cols-2 gap-4">
              {([
          ...
          <div className="rounded-[12px] border px-4 py-3"
            ...
            <p className="text-[11.5px] font-semibold mb-1" style={{ color: '#183153' }}>💡 Example</p>
...
function MemberDrawer({ guestId, prog, onClose }: { guestId: string; prog: any; onClose: () => void }) {
  ...
  const ACTION_BUTTONS = [
    { key: 'award'  as const, label: 'Award',  Icon: Plus,         bg: 'var(--rp-teal-bg)', color: '#183153' },
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[420px] flex-col bg-white dark:bg-white/5 shadow-2xl"
        ...
        <div className="flex items-center justify-between border-b px-6 py-4 shrink-0"
          ...
          <p className="font-display text-[16px] font-semibold text-[#183153] dark:text-[#f8fafc]">Member Profile</p>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[16px] font-semibold text-[#183153] dark:text-[#f8fafc]">{guest.firstName} {guest.lastName}</p>
                  <p className="text-[12px] text-[#64748b] dark:text-[#a9c1d0]">{guest.email}</p>
              ...
              {!account ? (
                <div className="flex flex-col items-center rounded-[14px] border border-dashed p-6 text-center"
                  style={{ borderColor: 'rgba(24,49,83,0.3)', background: 'var(--rp-teal-soft)' }}>
                  <Star className="h-8 w-8 mb-2" style={{ color: '#183153' }} />
                  <p className="text-[13px] font-medium mb-1 text-[#183153] dark:text-[#f8fafc]">Not enrolled yet</p>
                  <p className="text-[12px] mb-4 text-[#64748b] dark:text-[#a9c1d0]">Enroll this guest to start tracking points</p>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-[12px] p-4 text-center"
                      ...
                      <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[#475569] dark:text-[#9db4c4]">Balance</p>
                      <p className="text-[28px] font-black tracking-[-0.02em] mt-1" style={{ color: '#183153' }}>
                        {account.points.toLocaleString()}
                      </p>
                      <p className="text-[11px]" style={{ color: '#183153' }}>points</p>
                      {prog && <p className="text-[11.5px] font-medium mt-1" style={{ color: '#183153' }}>≈ {formatCurrency(account.points / prog.redemptionRate)}</p>}
                    </div>
                    <div className="rounded-[12px] p-4 text-center"
                      style={{ background: 'var(--rp-surface-2)' }}>
                      <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[#64748b] dark:text-[#a9c1d0]">Lifetime</p>
                      <p className="text-[28px] font-black tracking-[-0.02em] mt-1 text-[#183153] dark:text-[#f8fafc]">
                        {account.lifetimePoints.toLocaleString()}
                      </p>
                      <p className="text-[11px] text-[#64748b] dark:text-[#a9c1d0]">total pts</p>
                      <p className="text-[11.5px] mt-1 text-[#64748b] dark:text-[#a9c1d0]">{account._count?.transactions ?? 0} transactions</p>
                    </div>
                  </div>
                  ...
                  {nextTierPts && prog ? (
                    <div className="rounded-[12px] border p-4" style={{ background: 'var(--rp-surface-2)', borderColor: 'var(--rp-border)' }}>
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-[12px] font-medium text-[#183153] dark:text-[#f8fafc]">
                          Progress to {TIER_CONFIG[nextTierPts.label as keyof typeof TIER_CONFIG]?.icon} {nextTierPts.label}
                        </p>
                        <p className="text-[11.5px] text-[#64748b] dark:text-[#a9c1d0]">{nextTierPts.needed.toLocaleString()} pts to go</p>
                      </div>
                      {(() => {
                        ...
                        return (
                          <div className="h-[6px] w-full rounded-full" style={{ background: '#e8e5e0' }}>
                            <div className="h-[6px] rounded-full transition-all" style={{ background: '#183153', width: `${pct}%` }} />
                          </div>
                        );
                      })()}
                    </div>
                    ...
                    <div className="flex items-center gap-2.5 rounded-[12px] border px-4 py-3"
                      style={{ background: 'var(--rp-btn-accent)', borderColor: 'rgba(24,49,83,0.5)' }}>
                  ...
                  {mode && (
                    <div className="rounded-[12px] border p-4 space-y-3"
                      ...
                      <p className="text-[12px] font-semibold text-[#183153] dark:text-[#f8fafc]">
                  ...
                  {account.transactions?.length > 0 && (
                    <div>
                      <p className="text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[#64748b] mb-3 flex items-center gap-1.5">
                          ...
                          return (
                            <div key={tx.id} className="flex items-center justify-between rounded-[10px] border px-3 py-2.5"
                              ...
                              <div className="min-w-0">
                                <p className="text-[13px] truncate text-[#183153] dark:text-[#f8fafc]">{tx.description}</p>
            ...
            <p className="text-center text-[13px] text-[#64748b] dark:text-[#a9c1d0]">Guest not found</p>
          )}
        </div>
      </div>
    </>
  ...
  return (
    <PageShell gap={6}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'Members',         value: total,                                          icon: Users,  color: '#183153', bg: 'var(--rp-teal-bg)' },
          ...
          { label: 'Platinum',        value: tierCounts['PLATINUM'] ?? 0,                   icon: Crown,  color: 'var(--rp-btn-accent-text)', bg: '#183153' },
            ...
            {isLoading ? (
              ...
              <div className="py-16 flex flex-col items-center gap-3">
                <div className="flex h-16 w-16 items-center justify-center rounded-full" style={{ background: 'var(--rp-teal-bg)' }}>
                  <Star className="h-8 w-8" style={{ color: '#183153' }} />
                </div>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-[11.5px] truncate text-[#64748b] dark:text-[#a9c1d0]">{acc.guest.email}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3"><TierBadge tier={acc.tier} /></td>
                        <td className="px-4 py-3 text-right text-[13px] font-semibold" style={{ color: '#183153' }}>
                          {acc.points.toLocaleString()}
                        </td>
                        <td className="px-4 py-3 text-right text-[13px] text-[#64748b] dark:text-[#a9c1d0]">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-[14px] border bg-white dark:bg-white/5 p-5"
            ...
            <div className="flex items-center gap-2 mb-4">
              <TrendingUp className="h-4 w-4" style={{ color: '#183153' }} />
              <p className="text-[13px] font-semibold text-[#183153] dark:text-[#f8fafc]">Tier Distribution</p>
            </div>
            <div className="space-y-3">
                ...
                return (
                  <div key={tier}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[12px] font-medium text-[#183153] dark:text-[#f8fafc]">{cfg.icon} {cfg.label}</span>
                      <span className="text-[11.5px] text-[#64748b] dark:text-[#a9c1d0]">{count} ({pct}%)</span>
                    </div>
          ...
          <div className="rounded-[14px] border bg-white dark:bg-white/5 p-5"
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-[13px] font-semibold text-[#183153] dark:text-[#f8fafc]">Top Members</p>
            </div>
            {leaderboard.length === 0 ? (
              <p className="text-[12px] text-center py-4 text-[#64748b] dark:text-[#a9c1d0]">No members yet</p>
            ) : (
                ...
                {leaderboard.slice(0, 5).map((acc: any, i: number) => (
                  <div key={acc.id}
                    ...
                    <span className="text-[14px] w-5 text-center shrink-0">
                      {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : <span className="text-[12px] text-[#64748b] dark:text-[#a9c1d0]">{i + 1}.</span>}
                    </span>
                    ...
                    <span className="text-[13px] font-bold shrink-0" style={{ color: '#183153' }}>
                      {acc.lifetimePoints.toLocaleString()}
                    </span>
                  </div>
                ))}
          ...
          {prog?.isEnabled && (
            <div className="rounded-[14px] border px-5 py-4"
              style={{ background: 'var(--rp-teal-bg)', borderColor: 'rgba(24,49,83,0.2)' }}>
              <p className="text-[12px] font-semibold mb-1.5" style={{ color: '#183153' }}>💡 Earn Rate</p>
