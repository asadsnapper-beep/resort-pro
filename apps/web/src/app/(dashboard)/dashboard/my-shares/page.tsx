...
export default function MySharesPage() {
  ...
  if (!me) return (
    <div className="max-w-2xl rounded-[14px] border bg-white p-8 text-center" style={{ borderColor: 'var(--rp-border)' }}>
      <p className="text-[13.5px] text-[#64748b] dark:text-[#a9c1d0]">Kono shareholder profile paoa jayni. Owner-er sathe jogajog koro.</p>
    </div>
  );
  ...
  return (
    <PageShell gap={6} className="mx-auto max-w-3xl">
      ...
      {/* Summary card */}
      <div className="rounded-[14px] p-6 text-white" style={{ background: '#183153', boxShadow: '0 1px 6px rgba(0,0,0,0.08)' }}>
        <div className="flex items-center justify-between mb-5">
          <div>
            <p className="text-[10.5px] font-semibold uppercase tracking-[0.09em] text-[#698599]">Ownership</p>
            <p className="mt-1 text-[36px] font-semibold leading-none tracking-[-0.03em] text-[#ece7df]">{me.ownershipPercent}%</p>
          </div>
          <div className="text-right">
            <p className="text-[10.5px] font-semibold uppercase tracking-[0.09em] text-[#698599]">Joined</p>
            <p className="mt-1 text-[13.5px] text-[#f8fafc]">{new Date(me.joinedAt).toLocaleDateString('en-GB', { dateStyle: 'medium' })}</p>
          </div>
        </div>
        <div className="rounded-[10px] p-4" style={{ background: 'rgba(255,255,255,0.06)' }}>
          <p className="text-[11px] text-[#a9c1d0]">This month&apos;s estimated share (based on ৳{me.netProfitThisMonth.toLocaleString()} net profit)</p>
          <p className="mt-1 text-[24px] font-semibold text-[#ece7df]">{formatCurrency(me.estimatedShareThisMonth)}</p>
          <p className="mt-1 text-[11px] text-[#698599]">Estimate — actual payout may differ, owner confirm korbe</p>
        </div>
      </div>
      ...
      <div className="rounded-[14px] border bg-white overflow-hidden" style={{ borderColor: 'var(--rp-border)', boxShadow: '0 1px 6px rgba(0,0,0,0.04)' }}>
        <div className="px-5 py-4" style={{ background: 'var(--rp-surface-2)', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
          <h2 className="text-[13.5px] font-semibold text-[#183153] dark:text-[#f8fafc]">Payout History</h2>
        </div>
        {payoutsLoading ? (
          <div className="flex h-24 items-center justify-center">
            <Loader2 className="h-5 w-5 animate-spin" style={{ color: '#aac0d0' }} />
          </div>
        ) : payouts.length === 0 ? (
          <div className="flex flex-col items-center gap-2 py-14 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-full" style={{ background: 'var(--rp-surface-3)' }}>
              <Wallet className="h-7 w-7 text-[#94a3b8] dark:text-[#7f99ab]" />
            </div>
            <p className="text-[13px] text-[#64748b] dark:text-[#a9c1d0]">Ekhono kono payout record kora hoyni.</p>
          </div>
        ) : (
          <div>
            {payouts.map((p) => (
              <div key={p.id} className="flex items-center gap-4 px-5 py-4" style={{ borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13.5px] font-medium text-[#183153] dark:text-[#f8fafc]">{formatCurrency(p.amount)}</p>
